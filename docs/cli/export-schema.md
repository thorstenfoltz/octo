...
print it to stdout. Only the column list is read — no rows are
...
`-f` / `--format` does **not** apply — the output is source code, not a
...
| `--target` value       | Output                                                                          |
|------------------------|---------------------------------------------------------------------------------|
| `postgres` *(default)* | `CREATE TABLE` — Postgres dialect, double-quoted identifiers.                   |
| `mysql`                | `CREATE TABLE` — MySQL dialect, backtick identifiers, `UNSIGNED` types.         |
| `sqlite`               | `CREATE TABLE` — SQLite dialect, affinity types (INTEGER/REAL/TEXT/BLOB).       |
| `databricks`           | `CREATE TABLE` — Databricks (Spark SQL / Delta) dialect, backticks, `STRING`.   |
| `snowflake`            | `CREATE TABLE` — Snowflake dialect, double quotes, `VARCHAR` / `TIMESTAMP_NTZ`. |
...
GUI [Schema Export](../usage/schema-export.md#type-mapping) page — the
